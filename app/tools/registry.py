...
class ToolRegistry:
    ...
    def __init__(self):
        ...
        Auto-registration pattern - tools available immediately after instantiation.
    ...
    def register(self, name: str, func: Callable) -> None:
        ...
        Simple dictionary storage - overwrites existing tools with same name.
    ...
    def _register_default_tools(self):
        ...
        """
        # Register all code review workflow tools
...
def extract_functions(state, **kwargs) -> Dict[str, Any]:
    ...
    Uses regex to match 'def function_name():' patterns, counts newlines for line numbers.
    """
    code = state.get("code", "")
    ...
    # Regex pattern to match function definitions: def function_name(params):
    ...
    for match in re.finditer(func_pattern, code):
        functions.append({
            "name": match.group(1),  # Extract function name from regex group
...
def check_complexity(state, **kwargs) -> Dict[str, Any]:
    ...
    base complexity of 1 + count of if/for/while keywords.
    """
    code = state.get("code", "")
    ...
    # Calculate complexity for each discovered function
    for func in functions:
        ...
        complexity_scores.append({
            ...
            "level": "high" if complexity > 5 else "low"  # Simple threshold
        })
...
def detect_issues(state, **kwargs) -> Dict[str, Any]:
    ...
    for i, line in enumerate(lines, 1):
        ...
        # Check for overly long lines (style issue)
        if len(line) > 100:
            issues.append({
                "type": "style", "line": i,
        ...
        # Check for functions missing docstrings (documentation issue)
        if stripped_line.startswith('def ') and ':' in line:
            # Look ahead a few lines for docstring
            has_docstring = False
        ...
        # Check for TODO/FIXME comments (maintenance issue)
    ...
    # Count issues by severity for summary statistics
...
def suggest_improvements(state, **kwargs) -> Dict[str, Any]:
    ...
    Rule-based system - different thresholds trigger different suggestion types.
    """
    ...
    # Generate suggestions based on function complexity
    ...
    # Suggest documentation improvements if many functions lack docstrings
    ...
    # Suggest code formatting if many style issues
...
def calculate_quality_score(state, **kwargs) -> Dict[str, Any]:
   ...
   Starts with perfect score (10), deducts points for complexity/issues, adds bonus for structure.
    """
    ...
    # Deduct points based on complexity levels
    ...
    # Bonus points for having structured code (functions)
    if function_count > 0:
        base_score += 1
